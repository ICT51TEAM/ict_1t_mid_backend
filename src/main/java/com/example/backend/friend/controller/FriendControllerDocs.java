...
public interface FriendControllerDocs {
    ...
    ResponseEntity<List<FriendResponseDto>> getPendingRequests(@AuthenticationPrincipal Long userId);

    @Operation(summary = "상대방에게 친구요청 발송", description = "특정 사용자에게 친구 요청을 보냅니다.")
    ResponseEntity<String> sendFriendRequest(@AuthenticationPrincipal Long userId, @RequestBody FriendRequestDto requestDto);

    @Operation(summary = "친구 요청 수락 처리", description = "받은 친구 요청을 수락합니다.")
    ResponseEntity<String> acceptFriendRequest(@AuthenticationPrincipal Long userId, @PathVariable Long friendshipId);

    @Operation(summary = "친구 요청 거절 처리", description = "받은 친구 요청을 거절 상태로 변경합니다.")
    ResponseEntity<String> rejectFriendRequest(@AuthenticationPrincipal Long userId, @PathVariable Long friendshipId);

    @Operation(summary = "등록된 친구 삭제", description = "이미 등록된 친구를 친구 목록에서 삭제합니다.")
    ResponseEntity<String> deleteFriend(@PathVariable Long friendId, @AuthenticationPrincipal Long userId);

    @Operation(summary = "유저 닉네임 검색", description = "특정 키워드(닉네임)를 포함하는 유저 목록을 검색합니다.")
    ResponseEntity<List<UserSearchDto>> searchUsers(@RequestParam("q") String query, @AuthenticationPrincipal Long userId);
}
