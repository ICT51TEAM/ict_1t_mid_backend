...
public class UserService {
    ...
    public void deleteAccount(Long userid, String password) {
        ...
        // 일괄 삭제 로직 호출 (Native Query 1번으로 끝냄)
        try {
            userRepository.deleteAllUserData(user.getId());
            System.out.println("유저 {} 탈퇴 처리 완료"+ user.getId());
        } catch (Exception e) {
        	System.out.println("탈퇴 처리 중 DB 에러: {}"+ e.getMessage());
            throw new RuntimeException("탈퇴 처리 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.");
        }
    }
